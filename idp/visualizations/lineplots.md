...
Line Plots are likely the most common plot you'll create in IDP. It is fantastic for <a href='geospatial.html#correlations-and-coefficient-of-determination'>showing correlations</a>. While line plots are not complicated, it can be frustrating to draw multiple lines as you intend. Furthermore, there are many different API available to draw lines and they accept different named arguments (which is frustrating!).
...
to the "Min & Max" plot shown below.   
...
Here we will look at an example where the the units gets in the way of our plot. Then, we will address it using `twinx`. [Online example](https://matplotlib.org/stable/gallery/spines/multiple_yaxis_with_spines.html)  
...
We also use `loc` to do a time-slice of the data which is easy because our DataFrame is a <a href='https://cse163.github.io/book/module-3-pandas/lesson-9-time-series/time-series.html' target='_blank'>TimeSeries.</a>
